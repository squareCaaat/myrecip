...
class TestAuthRoutes:
    ...
    def test_register_duplicate_email(
        self, test_client, created_user_integration, sample_user_data
    ):
        """중복 이메일 회원가입 실패 테스트"""
        # Given
    ...
    def test_login_success(
        self, test_client, created_user_integration, sample_login_data
    ):
        """로그인 성공 테스트"""
        # Given
    ...
    def test_login_wrong_email(self, test_client, created_user_integration):
        """잘못된 이메일 로그인 실패 테스트"""
        # Given
        wrong_data = {"email": "wrong@example.com", "password": "password123"}
    ...
    def test_login_wrong_password(
        ...
        """잘못된 비밀번호 로그인 실패 테스트"""
        # Given
        wrong_data = {"email": sample_user_data["email"], "password": "wrongpassword"}
    ...
    def test_get_current_user_success(
        self, test_client, auth_headers_integration, created_user_integration
    ):
        """현재 사용자 정보 조회 성공 테스트"""
        # When
        with patch(
            "app.utils.redis_client.redis_client.get_token",
            return_value=auth_headers_integration["Authorization"].split(" ")[1],
        ), patch(
            "app.utils.redis_client.redis_client.is_token_blacklisted",
            return_value=False,
        ):
    ...
    def test_validate_token_success(
        ...
        with patch(
            ...
            response = test_client.get(
                "/api/auth/validate-token", headers=auth_headers_integration
            )
    ...
    def test_logout_success(
        ...
        with patch(
            ...
            "app.utils.redis_client.redis_client.delete_token", return_value=True
        ), patch(
            "app.utils.redis_client.redis_client.blacklist_token", return_value=True
        ):
            response = test_client.post(
                "/api/auth/logout", headers=auth_headers_integration
            )
...
class TestUserRoutes:
    ...
    def test_get_my_profile_success(
        ...
        with patch(
            ...
            response = test_client.get(
                "/api/users/me", headers=auth_headers_integration
            )
    ...
    def test_update_my_profile_success(
        self,
        test_client,
        auth_headers_integration,
        created_user_integration,
        sample_user_update_data,
    ):
        """내 프로필 수정 성공 테스트"""
        # Given
        ...
        with patch(
            ...
            response = test_client.put(
                "/api/users/me", json=update_data, headers=auth_headers_integration
            )
    ...
    def test_update_my_profile_partial(
        ...
        with patch(
            ...
            response = test_client.put(
                "/api/users/me", json=partial_update, headers=auth_headers_integration
            )
