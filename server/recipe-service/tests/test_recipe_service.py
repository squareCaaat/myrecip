...
class TestRecipeService:
    ...
    async def test_create_recipe_success(
        self, test_db_session, sample_recipe_data, test_user_id
    ):
        """레시피 생성 성공 테스트"""
        # Given
    ...
    @pytest.mark.asyncio
    async def test_create_recipe_invalid_user_id(
        self, test_db_session, sample_recipe_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_recipes_by_user_success(
        self, test_db_session, created_recipe, test_user_id
    ):
        ...
        # When
        result = await recipe_service.get_recipes_by_user(
            test_user_id, page=1, per_page=10
        )
    ...
    async def test_get_recipes_by_user_pagination(self, test_db_session, test_user_id):
        ...
        # When
        result = await recipe_service.get_recipes_by_user(
            test_user_id, page=1, per_page=3
        )
    ...
    @pytest.mark.asyncio
    async def test_get_recipe_by_id_no_permission(
        self, test_db_session, created_recipe
    ):
    ...
    async def test_update_recipe_success(
        self, test_db_session, created_recipe, test_user_id, sample_recipe_update_data
    ):
        """레시피 수정 성공 테스트"""
        # Given
        recipe_service = RecipeService(test_db_session)
        ...
        result = await recipe_service.update_recipe(
            recipe_id, update_data, test_user_id
        )
    ...
    async def test_update_recipe_not_found(
        self, test_db_session, test_user_id, sample_recipe_update_data
    ):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await recipe_service.update_recipe(
                non_existent_id, update_data, test_user_id
            )
    ...
    @pytest.mark.asyncio
    async def test_get_shared_recipe_success(
        self, test_db_session, created_recipe, test_user_id
    ):
        """공유된 레시피 조회 성공 테스트"""
        # Given
