...
class ImageService:
    ...
    async def _create_cocktail_visualization(
        ...
        # 글래스 외곽선 그리기
        glass_coords = [
            glass_x, glass_y + glass_height,  # 하단 좌
            glass_x, glass_y + 50,            # 상단 좌 (약간 위)
            glass_x + glass_width, glass_y + 50,  # 상단 우
            glass_x + glass_width, glass_y + glass_height  # 하단 우
        ]
        ...
        for ingredient in ingredients:
            ...
            # 색상 파싱 (기본값 사용)
            color = ingredient.color if ingredient.color and ingredient.color != "#000000" else self._get_default_color(ingredient.name)
            
            # 레이어 그리기
            layer_coords = [
                glass_x + 5, current_height,
                glass_x + 5, current_height - actual_layer_height,
                glass_x + glass_width - 5, current_height - actual_layer_height,
                glass_x + glass_width - 5, current_height
            ]
        ...
        try:
            ...
            font_size = 20
            title_text = recipe.title[:20] + "..." if len(recipe.title) > 20 else recipe.title
        ...
        # 재료 목록 추가
        ingredients_text = "\n".join([f"• {ing.name}: {ing.amount}{ing.unit}" for ing in ingredients[:5]])
    ...
    def _get_default_color(self, ingredient_name: str) -> str:
        ...
        # 기본 색상들
        default_colors = ['#87CEEB', '#DDA0DD', '#98FB98', '#F0E68C', '#FFB6C1', '#87CEFA']
