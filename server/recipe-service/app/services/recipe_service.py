...
from app.models.schemas import (
    ...
    IngredientCreate, ShareLinkResponse
)
import uuid
...
class RecipeService:
    ...
    async def get_recipes_by_user(
        ...
        # 전체 개수 조회
        total_stmt = select(func.count(Recipe.recipe_id)).where(Recipe.user_id == user_uuid)
    ...
    async def create_share_link(self, recipe_id: str, user_id: str) -> ShareLinkResponse:
        """공유 링크 생성"""
        try:
            recipe_uuid = uuid.UUID(recipe_id)
            user_uuid = uuid.UUID(user_id)
        ...
        return ShareLinkResponse(
            ...
            share_url=f"/shared/{recipe.share_id}"
        )
    ...
    async def _get_recipe_with_ingredients(self, recipe_id: uuid.UUID) -> Optional[RecipeResponse]:
