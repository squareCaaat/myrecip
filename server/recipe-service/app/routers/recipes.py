...
async def get_visual_image(recipe_id: str, db: AsyncSession = Depends(get_db)):
    """
    시각화 이미지 생성 및 반환
    ...
    return {"message": "시각화 이미지가 생성되었습니다.", "visual_url": visual_url}


# 공유 레시피 조회 (인증 불필요)
@router.get("/shared/{share_id}", response_model=RecipeResponse)
async def get_shared_recipe(share_id: str, db: AsyncSession = Depends(get_db)):
    """
    공유된 레시피 조회
