...
def check_test_redis():
    ...
    try:
        import redis
        client = redis.Redis(host='localhost', port=6379, db=1, decode_responses=True, password='password123!')
        client.ping()
        print("Redis 연결 성공")
        return True
    except Exception as e:
...
def run_docker_tests(service=None):
    """Docker 환경에서 테스트 실행"""
    if service:
        cmd = ['docker-compose', '-f', 'docker-compose.test.yml', 'up', '--build', f'{service}-test', '--abort-on-container-exit']
...
def main():
    ...
    parser.add_argument('--service', choices=['user-service', 'recipe-service', 'all'], 
                       default='all', help='실행할 서비스 (기본값: all)')
    parser.add_argument('--type', choices=['unit', 'integration', 'all'], 
                       default='all', help='실행할 테스트 타입 (기본값: all)')
