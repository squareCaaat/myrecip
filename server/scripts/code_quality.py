...
def format_code(target_dirs, fix=False):
    ...
    for target_dir in target_dirs:
        ...
        black_result = run_command(
            black_cmd,
            f"Black 포맷팅 {'적용' if fix else '검사'} - {target_dir}",
            check=False
        )
...
def main():
    ...
    parser.add_argument('--dirs', nargs='*', default=['user-service', 'recipe-service', 'api-gateway'],
                      help='검사할 디렉터리 목록')
